...
**Stimulus-parameter cleanups (VBN-only).** Two redundancies/noise sources are cleaned up for every VBN visual epoch: (1) the per-presentation **`stimulus_name`** parameter is dropped — it duplicates `stimulus_template_name` and the epoch's own `stimulus_name` fields; and (2) the **`image_set`** is taken from a single source, the presentation table's own `image_set` column, so the session-table `image_set` (which is NaN for gratings-stage sessions and would otherwise disagree) is not repeated. The jittery per-presentation **`duration`** column is summarized: rounded to the millisecond and de-duplicated, kept as a single scalar when effectively constant (e.g. a fixed `0.25 s` flash), otherwise **dropped** — a genuinely per-trial `duration` (e.g. the 100+ variable-length change-detection trials) is not worth a 100-value list in the metadata and remains available in the NWB intervals table. These three behaviors are **specific to VBN** (opt-in flags on the shared `get_visual_stimulation_parameters` / `convert_intervals_to_visual_stimulus_epoch` helpers): only the VBN presentation tables carry a `duration` column, and the Visual Coding and Visual Behavior Ophys pipelines keep their existing parameter shape (redundant `stimulus_name` and session-derived `training_protocol_name` / `curriculum_status`) unchanged.
