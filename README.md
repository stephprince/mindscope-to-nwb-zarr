...
- **Placeholder imaging-config values (documented, not fail-loud).** Two-photon laser power was adjusted per session and not recorded, so `LaserConfig.power` is left `None` and the required `Plane.power` is set to the sentinel `-1` percent. For multi-plane (mesoscope) coupled planes, `power_ratio` (`1.0`) and, for an unpaired QC-dropped plane, `coupled_plane_index` (`-1`) are placeholders pending the power-sharing records (`TODO`s in `acquisition_behavior_ophys.py`).
...
**Warnings and errors (full-dataset run).** The last completed full run generated **all 4782 sessions with 0 errors** (3416 BEH behavior-box + 869 CAM2P single-plane + 497 MESO mesoscope), each producing the full five-file set that validates against `aind-data-schema`. Its warnings were all benign/expected: upstream `aind-data-schema` coordinate-system / `Device.manufacturer` deprecations, the per-`Code` "no commit_hash/version" reproducibility note (the Visual Behavior stimulus code has no recorded version), the metadata-service legacy-record fixes listed above, and the mesoscope's benign duplicate-component-name note. That run **predates** the two changes documented here — the change-detection `StimulusEpoch` is now emitted for the (previously epoch-less) ophys sessions, and the 720 imaging-rig `session_start_time` values that previously produced warn-only mismatches are now corrected to UTC and fail-loud on any unexplained mismatch — so the metadata will be **regenerated**; these counts will be refreshed after the re-run.
