...
**Ethics (IACUC) review id.** `acquisition.json`'s `ethics_review_id` is looked up per subject (6-digit mouse id) from `code/reference/ethics_review_ids.csv`, which covers **65 of the 81** Visual Behavior Neuropixels subjects. For the **16 uncovered subjects below**, the lookup warns and leaves `ethics_review_id` **None** rather than failing the session — so `ethics_review_id` is `None` for **all 721 metadata assets of these 16 subjects** (689 behavior-only + 32 ecephys; the `behavior sessions` column below counts all `behavior_sessions.csv` rows, which include the ecephys-linked ones). Extend `ethics_review_ids.csv` when the ids become available.
...
- **Missing ethics review id** — `No ethics_review_id on file for subject …; leaving it None` — **721** (689 behavior-only + 32 ecephys, the 16 subjects tabled above). Warn-and-continue by design.
