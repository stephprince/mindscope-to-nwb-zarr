...
- The `static_gratings` stimulus presentation table (under `nwb.stimulus`) appears incomplete in at least some sessions: it holds only the first few presentations rather than the full block. For example, in session 645474010 (`sub-613392734_ses-645474010-StimB`) the table has just 3 rows near the session start, even though the `epochs` table shows three separate `static_gratings` blocks totaling ~24 minutes. As a result, only the first static-gratings block has any per-presentation parameters; the later blocks have none. The coarse block timing in the `epochs` table is unaffected. This should be investigated and the full presentation table restored if the source data allows.
...
**Batch metadata generation.** AIND metadata was generated successfully for **all 58** Visual Coding Neuropixels sessions, each session NWB streamed from the public `allen-brain-observatory` S3 bucket over HTTPS (no download / no mount), by `code/scripts/run_all_vc_ephys.py`. Every session produced the full five-file set (data description, subject, acquisition, procedures, instrument): **58/58 succeeded, 0 errors** (~94 min, 3 workers). The cohort is 58 unique mice — 32 `brain_observatory_1.1` and 26 `functional_connectivity` sessions. Stimulus-epoch counts per session: 8 (×4), 9 (×46), 10 (×8) — no 0-epoch sessions. Use **≤3 workers**: every ephys subject hits the slow metadata-service raw-parse fallback, and each worker also streams a ~2.6 GB NWB and reads its spike times (for the acquisition end time).

The following warnings are expected and handled:

- **Metadata-service reconciliation** — 50 of 58 subjects fail client-side schema validation, so the raw service response is parsed and known gaps are patched. Genotype is **backfilled from the NWB** (all `"wt/wt"`) for 30 wildtype subjects (see below); null `maternal_genotype` (9 subjects) / `paternal_genotype` (18 subjects) are set to `""`; and the same 9 `functional_connectivity` sessions get the **anaesthesia-duration** (`0.0`) and **Craniotomy-position** (`'Left'` → `["Left"]`) procedures fixes.
- **Interleaved stimulus epochs (all 58 sessions)** — `get_stimulation_epochs` emits one epoch per stimulus intervals table spanning its full extent, so the interleaved Neuropixels stimulus blocks overlap in time. This is warned (not fatal) and not yet split into per-block epochs — a future-run improvement; all five files are still written normally.
- **Deprecated probe coordinate axis** — the probe-local `PROBE_RUFD` coordinate system uses a 4th `DEPTH` axis that `aind-data-schema` now deprecates; flagged for follow-up with the reference author. (The separate `coordinate_system` → `global_coordinate_system`/`local_coordinate_system` field deprecation has since been resolved in code; a residual read-time warning remains, emitted by an upstream `aind-data-schema` validator.)
...
**Checks that never triggered** — clean across all 58 sessions: no **sex**, **genotype** (both sources present), **date-of-birth** (>2-day), **species**, or **mouse-id** mismatch, and the metadata service was never unreachable. (A species mismatch would hard-fail the session; the others warn and keep the LIMS value.)
