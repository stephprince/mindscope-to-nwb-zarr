...
| `scripts/run_all_vc_ephys.py` | Batch-generate AIND metadata for all Visual Coding Neuropixels sessions (streams from S3; `--zip` writes one zip per session to `metadata_results/visual-coding-neuropixels-metadata-only`; use ≤3 workers) |
...
- **Added the AllenSDK per-unit channel-derived columns to the `units` table** so it carries the same channel-derived information the AllenSDK units table returns (which builds them by merging units to channels on `peak_channel_id`). The source NWB units table has only `peak_channel_id`; this joins each unit's peak channel from the published `channels.csv` and adds its **CCF position** (`anterior_posterior_ccf_coordinate` / `dorsal_ventral_ccf_coordinate` / `left_right_ccf_coordinate`), **brain area** (`ecephys_structure_acronym` + numeric `ecephys_structure_id`), and **on-probe geometry** (`probe_horizontal_position` / `probe_vertical_position` / `ecephys_probe_id`) — 8 columns, added by `_units_analysis_metrics.py` (`add_unit_channel_columns`). The waveform/quality metrics the AllenSDK units table also carries (e.g. `waveform_halfwidth`, `spread`, `velocity_*`, `snr`) are **already in the source NWB units table**, so they are left untouched. CCF coordinates and the numeric structure id are NaN where the peak channel is out of brain / unregistered (or absent from `channels.csv`); the probe geometry is **not** nulled by the CCF sentinel (a channel keeps its on-probe position even when unregistered to the CCF).
- **Not copied to the `units` table: `valid_data` and the probe hardware fields.** The AllenSDK units table also exposes a per-unit `valid_data` flag (whether the peak channel's data is usable). It is **not** on the `units` table — the Visual Coding `channels.csv` does not carry it — but **could be copied there in the future**; in the meantime it is retrievable per unit by joining the units table to the electrodes table (which does carry `valid_data`) on the peak channel, e.g.:
  ```python
  units = nwbfile.units.to_dataframe()
  electrodes = nwbfile.electrodes.to_dataframe()
  units["valid_data"] = electrodes.loc[units["peak_channel_id"], "valid_data"].values
  ```
  The probe hardware fields the AllenSDK units table denormalizes onto every unit (`name`, `sampling_rate`, `lfp_sampling_rate`, `has_lfp_data`, `phase`) are **intentionally not added** to the units table — they are accessible on the probe device (`EcephysProbe` / electrode group) in the NWB.
- **`local_index` is the *unit's* within-probe index — not a channel index.** The NWB units table's `local_index` (kept as-is from the source) is the **unit's** index within its probe. AllenSDK's `session.units` separately exposes **`channel_local_index`** — the within-probe index (0–383) of the unit's *peak channel* — which is **not stored in the NWB** and is not added here (it is derivable by joining `peak_channel_id` to the electrodes' channel index). Beware that AllenSDK's *project-cache* `get_units().local_index` is that **channel** index, not the unit's: the published `units.csv` omits the unit's `local_index`, so the units↔channels merge keeps the channel's (verified equal to each unit's peak-channel `local_index` for all 40,010 units). So `get_units().local_index` and the NWB units table's `local_index` mean different things.
...
- **Corrected the `electrodes` table `z` column (packaging-error fix).** The source NWBs have a packaging error in which the electrodes `z` column duplicates `y` (both hold the dorsal-ventral CCF coordinate) instead of the left-right (medial-lateral) coordinate. The conversion overwrites `z` with the true `left_right_ccf_coordinate` from `channels.csv` (joined on electrode id == channel id) so the electrodes table's `x`/`y`/`z` are AP/DV/LR as intended; the corrected `z` matches each unit's `left_right_ccf_coordinate` (both sourced from `channels.csv`). The `-1000` "not registered to CCF" sentinel used by `channels.csv` is mapped to **NaN** (matching the NWB's NaN for `x`/`y`), and electrodes absent from `channels.csv` also get NaN.
...
Pass **`--zip`** to bundle each session's five files into a single `<data asset name>.zip` in `code/metadata_results/visual-coding-neuropixels-metadata-only/` (named to match the Code Ocean metadata-only data asset the Zarr conversion pipeline mounts at `data/visual-coding-neuropixels-metadata-only`; the run report is written to `code/metadata_results/reports/visual_coding_ephys/`, a sibling directory, so the deliverable directory holds only the 58 per-session zips).
...
**Data description tags.** The asset `tags` are `mindscope` plus the Allen linkage ids present on the session row (`ecephys_session_id`, `behavior_session_id`) — which are the **only** place those ids live in the metadata, and are what the conversion pipeline reads to resolve the session's S3 files. Two columns are intentionally **not** tagged: `session_type` (already the `acquisition.acquisition_type` = `nwbfile.session_description`), and `project_code` (the ecephys-only value is the constant `"NeuropixelVisualBehavior"` for every session and duplicates `DataDescription.project_name`, so it adds no information).
