...
- **Eye-gaze (DLC screen-mapping) data added.** The Allen Brain Observatory has a DeepLabCut eye-tracking product — pupil position projected onto the stimulus monitor plus pupil/eye area — that is **not** in the v1 experiment NWBs and so was never carried into DANDI. It lives in a separate S3 prefix, `s3://allen-brain-observatory/visual-coding-2p/ophys_eye_gaze_mapping/`, as one `<experiment_id>_<ophys_session_id>_eyetracking_dlc_to_screen_mapping.h5` (~20 MB) per experiment, present for **837 of the 1518** experiments. This conversion downloads that file (when present; a no-op for the other 681) and attaches it to `nwb.processing['behavior']` via `run_conversion.py` → `data_conversion/visual_coding_ophys/_eye_gaze_mapping.py::add_eye_gaze_mapping`:
  - `EyeTracking_dlc_screen_mapping` — `SpatialSeries` `pupil_location_dlc_screen_mapping` (gaze on the monitor, **cm**);
  - `CompassDirection_dlc_screen_mapping` — `SpatialSeries` `pupil_location_spherical_dlc_screen_mapping` (angular gaze, **degrees**);
  - `PupilTracking_dlc_screen_mapping` — `TimeSeries` `pupil_area_*` and `eye_area_*` (**pixels²**).
  Each quantity is stored in both a **blink-filtered** (`new_*`, NaN during blinks/dropped frames, ~43% of frames) and an **unfiltered** (`raw_*`) variant, all sharing the file's `synced_frame_timestamps` (seconds, session master clock). The `_dlc_screen_mapping` suffix keeps this product distinct from — and coexisting with — the older v1-embedded eye tracking that DANDI already carries for an overlapping subset (see the metadata note below); **178** of the 837 sessions have both. The H5 is read directly with h5py (no AllenSDK, so it stays in the conversion env) and downloaded at conversion time, so there is no cached asset for it (unlike `static_gratings`). The parse matches AllenSDK's authoritative reader (`read_eye_gaze_mappings` / `create_eye_gaze_mapping_dataframe`, used by `BrainObservatoryCache.get_ophys_pupil_data`) exactly — same datasets, column-name-based x/y indexing, and units. Note: the Allen Institute flags this DLC product as **provisional** ("obtained using a new eye tracking algorithm and is in the process of being validated"); each series description carries that caveat.
- **2p-frame-aligned eye tracking added (complementary product).** In addition to the gaze-mapping H5 above, the conversion adds a *second* eye-tracking product: the `eye_tracking.npy` served by `BrainObservatoryCache.get_eye_tracking()` (columns documented by AllenSDK PR #2740: `[2p_frame_number, eye_area_cm², pupil_area_cm², azimuth_deg, altitude_deg]`), present for **818 of 1518** experiments. Provenance: these files were **generated by the [`AllenInstitute/visual-coding-saccades`](https://github.com/AllenInstitute/visual-coding-saccades) project** (`data_loading/package_eye_data.py`) from Allen LIMS raw eye-tracking + time-sync files, resampled to the 2-photon frames, and published in the versioned `visual-coding-ophys-data` S3 release (a different, us-west-2, S3CloudCache-managed bucket than the raw `allen-brain-observatory` dump). `run_conversion.py` → `_eye_tracking_2p.py::add_eye_tracking_2p` downloads it env-free via quilt3 (no AllenSDK) and attaches, under **`_dlc`** names timestamped by the session's dF/F (2-photon) frame clock:
  - `CompassDirection_dlc` — `SpatialSeries` `pupil_location_spherical_dlc` (pupil gaze azimuth/altitude, **degrees**);
  - `PupilTracking_dlc` — `TimeSeries` `pupil_area_dlc` and `eye_area_dlc` (**cm²**).
  This is the *same underlying DLC tracking* as the gaze-mapping H5 (their pupil areas match to corr 0.998, up to the px²→cm² conversion) but a **different pipeline with a different gaze coordinate convention** — so both are kept, under distinct suffixes (`_dlc` vs `_dlc_screen_mapping`), and each `_dlc` series description records the `visual-coding-saccades` provenance and provisional status. The frame-index column indexes the dF/F frames, so no timestamp column is stored separately.
  - **Axis convention (important when comparing the two products).** The two use **swapped** gaze axes: product C's **azimuth** (horizontal) matches product B's **`y_pos_deg`** (corr 0.98, matching median/spread), and C's **altitude** (vertical) matches B's **`x_pos_deg`**. Consequently B's `x_pos`/`x_pos_deg` is the *vertical/altitude* axis and B's `y_pos`/`y_pos_deg` is the *horizontal/azimuth* axis — so B's `x_pos_deg` having a narrow range (e.g. ~3° vs ~11° for `y_pos_deg`) is expected (mice move their eyes far less vertically), **not** a defect. The vertical axis additionally differs in offset/scale between the two pipelines (different geometry calibration). Neither source's columns are reordered; this note is the reconciliation.
- **Code Ocean run model / single-session gating.** The conversion is a Code Ocean *pipeline* where each parallel job mounts exactly **one** metadata zip from the `visual-coding-ophys-metadata-only` data asset (the 1518 zips from `run_all_vc_ophys.py --zip`), unzips it, reads the ophys experiment id from `data_description.json`, downloads that session's processed + raw NWBs from DANDI, applies all the fixes above, and writes `results/<session name>/<session name>.nwb.zarr`. The module constant **`TEST_ONLY_ZIP_NAME`** in `run_conversion.py` gates this: when set to a zip filename, only the job whose mounted zip matches does work and every other job is a no-op (empty placeholder). It is currently set to the alphabetically **first** zip (subject 221470) so the first Code Ocean run converts one session end-to-end before fanning out; set it to **`None`** for full production (every job converts its mounted zip). The name embeds the per-session packaging timestamp, so update it if the metadata zips are regenerated.
...
- **Session 839557629 — base-only Zarr (no LFP).** All 5 probes on this `functional_connectivity` session are flagged `has_lfp_data=False`, so no `probe_*_lfp.nwb` files exist (on S3 or DANDI:000022) and the Zarr contains no LFP/CSD. The spikes/units (1770 units), electrodes, and stimulus data are intact; the conversion completes quickly because the per-probe LFP files are the bulk of the data.
...
- **Eye/pupil tracking is present for a subset; camera videos are not.** Eye/pupil tracking **is** packaged in the DANDI NWBs for the sessions whose v1 source NWB had a `processing/brain_observatory_pipeline/EyeTracking` group (**363 of 1518**): the upstream converter copied it into `processing['behavior']` as `EyeTracking` (`SpatialSeries` `pupil_location`, unit m), `CompassDirection` (`pupil_location_spherical`, degrees), and `PupilTracking` (`pupil_size`, px), referenced to the monitor center. Separately, this repo's conversion adds the richer DLC eye-gaze-mapping product for **837** sessions (see the "Eye-gaze (DLC screen-mapping) data added" note under **Visual Coding Ophys** conversion above); combined, **1022 of 1518** sessions have some eye product. What is still **not** packaged, for any session, is the eye/body **camera video**, even though the cameras are described in the instrument.
