...
- Rechunked LFP data to have chunk shape `(500000, 8)` improve read and write performance when using Zarr storage and for more reliable write performance during conversion in a Code Ocean pipeline where publishing the Zarr store to S3 is limited in the number of COPY requests per second for an S3 prefix.
- Rechunked the raw 2p imaging data 
